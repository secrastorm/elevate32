...
)
{
    ...
    if (!(Unelevated && IsElevated())) {
        if (!ShellExecuteEx(&Shex)) {
            ...
            WCHAR Err[260];
            swprintf_s(Err, 260, L"%s could not be launched: %d", ApplicationName, GetLastError());
...
INT DispatchCommand(__in PCOMMAND_LINE_ARGS Args)
{
    ...
    if (Args->ShowHelp)
    {
        ...
        MessageBox(NULL, HelpMsg, L"", MB_OK);
        return EXIT_SUCCESS;
    }
    ...
    if (Args->StartComspec)
    {
        ...
        if (0 == GetEnvironmentVariable(L"COMSPEC", AppNameBuffer, _countof(AppNameBuffer)))
        {
            ...
            MessageBox(NULL, L"%%COMSPEC%% is not defined.", NULL, MB_OK + MB_ICONERROR);
            return EXIT_FAILURE;
        }
            ...
            Args->CommandLine)))
        {
            ...
            MessageBox(NULL, L"Creating command line failed.\n", NULL, MB_OK + MB_ICONERROR);
            return EXIT_FAILURE;
        }
    ...
    _In_     INT       nCmdShow)
{
    ...
    if (FALSE == VerifyVersionInfo(&OsVer, VerifyVersionFlags, VersionConditionMask))
    {
        ...
        MessageBox(NULL, L"This tool is for Windows Vista and above only.\n", NULL, MB_OK + MB_ICONERROR);
        return EXIT_FAILURE;
    }
    ...
    for (Index = 1; Index < Argc; Index++)
    {
            ...
            (Argv[Index][0] == L'-' || Argv[Index][0] == L'/'))
        {
            ...
            else
            {
                ...
                MessageBox(NULL, (L"Unrecognized Flag: " + std::wstring(FlagName)).c_str(), NULL, MB_OK + MB_ICONERROR);
                LocalFree(Argv);
                return EXIT_FAILURE;
            }
        }
        else
        {
            ...
            else
            {
                        ...
                        L" ")))
                {
                    ...
                    MessageBox(NULL, L"Command Line too long.", NULL, MB_OK + MB_ICONERROR);
                    LocalFree(Argv);
                    return EXIT_FAILURE;
                }
            }
            ...
            (!Args.StartComspec && Args.ApplicationName == NULL)))
    {
        ...
        MessageBox(NULL, L"Invalid arguments.", NULL, MB_OK + MB_ICONERROR);
        return EXIT_FAILURE;
    }

    //return DispatchCommand(&Args);
    INT Result = DispatchCommand(&Args);
    LocalFree(Argv);
    return Result;
}